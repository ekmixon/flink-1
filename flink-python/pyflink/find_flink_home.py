...
def _is_flink_home(path):
    flink_script_file = f"{path}/bin/flink"
    return len(glob.glob(flink_script_file)) > 0


def _is_apache_flink_libraries_home(path):
    flink_dist_jar_file = f"{path}/lib/flink-dist*.jar"
    return len(glob.glob(flink_dist_jar_file)) > 0
...
def _find_flink_home():
    ...
    try:
        ...
        flink_root_dir = os.path.abspath(f"{current_dir}/../../")
        build_target = glob.glob(
            f"{flink_root_dir}/flink-dist/target/flink-*-bin/flink-*"
        )
...
def _find_flink_source_root():
    ...
    """
    try:
        return os.path.abspath(f"{os.path.dirname(os.path.abspath(__file__))}/../../")
    except Exception:
        pass
